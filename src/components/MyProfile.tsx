  ...
  const handleSaveChanges = async (e: FormEvent) => {
    ...
    try {
      if (profilePhotoFile) {
        ...
        setUser((prevUser) => ({
          ...prevUser,
          profilePhoto: photoResponse.data.url,
        }));
      }
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSaveChanges}>
        ...
        {isEditing && (
          <button
            type="submit"
            className="mt-6 bg-rose-500 text-white px-4 py-2 rounded-lg"
          >
            Save Changes
          </button>
        )}
      ...
      {isPasswordModalOpen && (
        <Modal
          ...
          onSubmit={handlePasswordModalToggle}
          title="Change Password"
          actionLabel="Submit"
          body={<ChangePasswordModal onClose={handlePasswordModalToggle} />}
        />
      )}
    </div>
  );
